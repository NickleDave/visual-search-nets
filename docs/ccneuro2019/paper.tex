...
{
\bf
Selective attention mechanisms preferentially route some sensory inputs for further 
processing, while necessarily discarding others. Researchers studying visual
attention have often divided selective attention into overt and covert mechanisms. 
Overt mechanisms involve orienting the eyes and/or head, while covert mechanisms 
occur "centrally", in the brain, without overt orientation. Cognitive scientists have long
used a new classic form of visual search task to investigate theoretical mechanisms of 
selective covert visual attention. Here we build on previous studies that examine the 
parallels between the visual system and convolutional neural networks (CNNs), by testing 
whether CNNs exhibit human-like attentional effects while performing this visual search 
task. Following a typical approach, we first show that CNNs trained 
for image classification can show human-like attentional effects. Then we demonstrate 
that, by changing how we train the networks and the statistics of the training data, 
these effects can largely be eliminated, i.e., that CNNs are capable of performing the 
task without explicit attention mechanisms. We go on to show that manipulating the 
...
metrics. Commonly used attention mechanisms consist essentially of weightings 
that are learned by updating the weights so as to minimize a loss function, 
with some additionally using recurrence
[@huSqueezeandExcitationNetworks2017; @vaswaniAttentionAllYou2017].
This understanding of attention as a mechanism to improve accuracy or other metrics 
