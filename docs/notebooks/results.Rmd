...
### Convolutional neural networks and attentional mechanisms
Neural networks have in the past few years transitioned from being an academic area of artificial intellgience to being powerful tools for engineers working in areas such as image classification, speech-to-text, and automatic translation, to name but a few. In the 1980s, artificial neural networks trained with backpropagation gave rise to [Connectionist](https://en.wikipedia.org/wiki/Connectionism) models of cognition. Similar neural network models lie at the heart of the recent rise of deep learning, in large part because of advances in methods for training networks, and advances in technology such as GPUs that allow for parallelized training. When benchmarked with scalar metrics like accuracy on computer vision tasks such as object recognition, these deep neural networks appear capable of human or super-human levels of skill. The first major gains in accuracy were seen on image classification tasks such as ImageNet [@krizhevskyImagenetClassificationDeep2012; @simonyanVeryDeepConvolutional2014], and the deep neural networks that made these advances were built with what are known as convolutional layers. As the name implies, these layers perform a convolution-like operation by multiplying filters of learned weights with the input image, producing as outputs feature maps that can be further transformed by succeeding layers. Convolutional neural networks (CNNs) represent the state of the art in image recognition. Many mechanisms proposed to extend the abilities of CNNs have been given the name "attention", and these attention mechanisms typically involve some sort of recurrence [@baMultipleObjectRecognition2014; @mnihRecurrentModelsVisual2014; @xuShowAttendTell2015]. These mechanisms are arguably analogous to what is called **overt** attention in psychology and neuroscience. In the context of vision, overt attention describes movement of the eyes or head so that the high-acuity fovea is placed on the focus of attention. In contrast, **covert** attention is any mechanism that occurs internally to select features relevant for a task while discarding irrelevant features. Covert attention can be considered one type of **dynamic routing** of information thought to occur in the brain. Models of dynamic routing, inspired by studies of visual attention [@bobierAttentionalRoutingCircuit2011; @bobierUnifyingMechanisticModel2014a; @olshausenMultiscaleDynamicRouting1995; @olshausenNeurobiologicalModelVisual1993] have also formed the basis of attention-like mechanisms in applied research on neural networks [@tangLearningGenerativeModels2014]. The function of CNNs has not typically been considered within the context of this work on covert visual attention. 

### Convolutional neural networks as models of the visual system
Recently, many in neuroscience and cognitive science have returned to using neural network models to understand the brain, in part because of their resurgence. More specifically, many researchers have drawn parallels between the architecture of CNNs and the architecture of the visual system in the brain [@kriegeskorteDeepNeuralNetworks2015], in particular in humans and other primates where this system has been most thoroughly studied. Like CNNs, the visual system has a hierarchical structure, and is thought to function in part by performing transformations at each level of this hierarchy so that high-dimensional, low-level features are mapped into low-dimensional abstract representations. Several recent studies in the realm of cognitive neuroscience have measured representations in more directly tested the similarities between CNNs and the visual system, by comparing representations in both [@khaligh-razaviDeepSupervisedNot2014; @yaminsPerformanceoptimizedHierarchicalModels2014; @yaminsUsingGoaldrivenDeep2016]. Representations can be easily extracted from CNNs because they are "transparent box" models--i.e. we have access to the "activity" of the individual units from which these models are built when we "show" the CNNs a stimulus. In the same studies, brain activity was obtained with electrophysiological or functional imaging methods. Such studies have found in general that representations are similar. (Of course this comparison depends on methods for measuring representational similarity [@bobadilla-suarezMeasuresNeuralSimilarity2018; @diedrichsenRepresentationalModelsCommon2017], but this study does not address those methods.)  

### Visual search tasks for investigating attention
In spite of the many attentional mechanisms proposed for neural networks, and the many parallels and links between studies of vision in psychology, neuroscience, and artificial intelligence, little work has been done to relate the tasks and models used to investigate visual attention to neural networks. We are aware of only one study [@poderCapacityLimitationsVisual2017], which led us to the results that we report below as we replicated it. Like that study, we measured the performance of an exist CNN on a visual search task. In addition, we test whether our results hold for other CNN architectures, and we apply analyses from signal detection theory-based models of attention to understand our results. Our ultimate goal is to provide new methods for comparing neural network output with human behavior and of benchmarking proposed attentional mechansims. First we describe visual search tasks, and then we describe different types of cognitive models used to explain results obtained with these tasks.
We begin with background on visual search tasks. In the "classical" form of the visual search paradigm [@wolfeVisualSearch1998], experimenters present a subject with a search display on which there are discrete items, i.e. objects. The subject is asked to report whether a target is present among distractors. The experimenter sets the probability that the target will be present on any given trial. Experimenters measure the reaction time and/or accuracy of the subjects. Often, the results are presented as a plot of reaction time as a function of set size (the number of discrete items in stimulus). By fitting a line to the plot of reaction times as a function of set size and taking the slope of that line, experimenters produced estimates of processing speed, i.e. "seconds per item". Slopes are almost horizontal when subjects are shown visual search stimuli where targets differ from distractors in some clearly salient **feature**, e.g. a red target among green distractors, and slopes become much steeper when targets differ from distractors in some harder-to-distinguish **spatial conjunction** of features, e.g., a digital 2 among digital 5s. Some examples of these stimuli are shown below, generated with https://github.com/NickleDave/searchstims.
...
To test whether these effects resulted from the architecture of AlexNet or were more general, we repeated the simulations using the VGG16 architecture. We again saw a clear difference in set size effects when comparing accuracy of networks performing either feature search or spatial conjunction search.
...
We also again saw that the set size effect increased, as did difference in accuracy v. set size slopes for feature and spatial conjunction search, as the VGG16 architecture was trained for more epochs.
...
The Feature Integration Theory of visual search attributed set size effects to **serial** processing required to bind different types of features [@treismanFeatureintegrationAttention1980]. This conclusion made sense, based on the result that reaction time increased when discriminating targets from distractors required subjects to consider more than one feature, e.g. both color and orientation. However, later work showed that reaction time effects disappear as targets and distractors become more easily discriminable along one feature dimension [@duncanVisualSearchStimulus1989].  
Later researchers, using methods from psychophysics and signal-detection theory, carefully controlled for the discriminability of targets and distractors [@palmerPsychophysicsVisualSearch2000]. This work provided important alternative explanations for the results seen in visual search experiments: attentional effects can be explained in terms of completely parallel but noisy representations that are used by a decision rule [@ecksteinLowerVisualSearch1998; @ecksteinSignalDetectionModel2000; @palmerMeasuringEffectAttention1993; @palmerSetsizeEffectsVisual1994]. This approach also explains errors in terms of low thresholds used by decision making criteria.  We adapted these psychophysics methods to see if they offered additional insights into the attention-like effects we saw in CNNs.  
...
Here we see that the difference threshold goes up with set size, meaning more discriminable target-distractor pairs are required as set size increases. In signal detection theory terms, this result is consistent with any theory of attention where decisions are made with a low threshold. (For a detailed walkthrough of how we did this, please see [this notebook](./psychometric.ipynb).)
...
* epochs: [10 (as in Poder 2017), 50, 200, 400, 800]
