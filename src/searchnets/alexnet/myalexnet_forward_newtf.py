...
def alexnet(graph, x):
    ...
    conv1_in = conv(x, conv1W, conv1b, k_h, k_w, c_o, s_h, s_w,
    ...
    #fc6
    #fc(4096, name='fc6')
    input_shape = maxpool5.get_shape().as_list()[1:]
    n_input_units = np.prod(input_shape)
    n_output_units = 4096
    weights_shape = [n_input_units, n_output_units]
    fc6W = tf.get_variable(name='fc6W_weights',
                              shape=weights_shape)
    fc6b = tf.get_variable(name='fc6W_biases',
    ...
    #fc7
    #fc(4096, name='fc7')
    input_shape = fc6.get_shape().as_list()[1:]
    n_input_units = np.prod(input_shape)
    n_output_units = 4096
    weights_shape = [n_input_units, n_output_units]
    fc7W = tf.get_variable(name='fc7W_weights',
                              shape=weights_shape)
    fc7b = tf.get_variable(name='fc7W_biases',
                             initializer=tf.zeros(
                                 shape=[n_output_units]))
    ...
    #fc8
    #fc(1000, relu=False, name='fc8')
    input_shape = fc7.get_shape().as_list()[1:]
    n_input_units = np.prod(input_shape)
    n_output_units = 2
    weights_shape = [n_input_units, n_output_units]
    fc8W = tf.get_variable(name='fc8W__weights',
                              shape=weights_shape)
    fc8b = tf.get_variable(name='fc8W__biases',
                             initializer=tf.zeros(
                                 shape=[n_output_units]))
    fc8 = tf.nn.xw_plus_b(fc7, fc8W, fc8b)

    layers_list = [conv1, lrn1, maxpool1, conv2, lrn2, maxpool2,
                   conv3, conv4, conv5, maxpool5, fc6, fc7, fc8]
...
if __name__ == '__main__':
    ...
    graph = tf.Graph()
    x = tf.placeholder(tf.float32, (None,) + ALEXNET_INPUT_SHAPE)

    alxnt = alexnet(graph, x)

    t = time.time()
    output = sess.run(alexnet, feed_dict = {x:[im1,im2]})
    ################################################################################
